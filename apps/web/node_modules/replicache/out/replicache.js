...
${n}
Error:
`,t)}async function lu(t,e,n,o,r,s){y(o.replicacheFormatVersion===4),Wo(t);let{delegate:i,lc:a,wrapInOnlineCheck:l,wrapInReauthRetries:u,isPushDisabled:c,isPullDisabled:d}=r,p=i.clientID;if(p===e||t.lastServerAckdMutationID>=t.mutationID)return;let f=`Recovering mutations for ${e}.`;a.debug?.("Start:",f);let g=new Oe(n,ai,Io,B);try{if(await v(g,x=>x.setHead(N,t.headHash)),c()){a.debug?.(`Cannot recover mutations for client ${e} because push is disabled.`);return}let{pusher:m}=i,S="recoveringMutationsPush";if(!await l(async()=>{let{result:x}=await u(async(V,R)=>{at(g);let b=await Ut(V,g,R,await i.profileID,void 0,e,m,o.schemaVersion,Bn);return{result:b,httpRequestInfo:b?.httpRequestInfo}},S,a);return!!x&&x.httpRequestInfo.httpStatusCode===200},S)){a.debug?.(`Failed to recover mutations for client ${e} due to a push error.`);return}if(d()){a.debug?.(`Cannot confirm mutations were recovered for client ${e} because pull is disabled.`);return}let{puller:I}=i,C="recoveringMutationsPull",w;if(!await l(async()=>{let{result:x}=await u(async(V,R)=>{let b=await ti(await i.profileID,e,o.schemaVersion,I,V,g,s,R,!1);return{result:b,httpRequestInfo:b.httpRequestInfo}},C,a);return{pullResponse:w}=x,!!w&&x.httpRequestInfo.httpStatusCode===200},C)){a.debug?.(`Failed to recover mutations for client ${e} due to a pull error.`);return}return a.debug&&w&&(G(w)?a.debug?.(`Client ${p} cannot recover mutations for client ${e}. The client no longer exists on the server.`):K(w)?a.debug?.(`Version is not supported on the server. versionType: ${w.versionType}. Cannot recover mutations for client ${e}.`):a.debug?.(`Client ${p} recovered mutations for client ${e}.  Details`,{mutationID:t.mutationID,lastServerAckdMutationID:t.lastServerAckdMutationID,lastMutationID:w.lastMutationID})),await v(n,async x=>{let V=await q(x),R=V.get(e);if(!R)return V;Wo(R);let b=async J=>(await ye(J,x),J);if(G(w)||K(w)){let J=new Map(V);return J.delete(e),b(J)}if(y(w),R.lastServerAckdMutationID>=w.lastMutationID)return V;let L=new Map(V).set(e,{...R,lastServerAckdMutationID:w.lastMutationID});return b(L)})}catch(m){_t(m,a,f,i)}finally{await g.close(),a.debug?.("End:",f)}}async function uu(t,e,n,o){let r=o(t.name),s=new Ie(r,z,B);try{await li(t,e,s,n)}finally{await s.close()}}function li(t,e,n,o){return t.replicacheFormatVersion>=5?pu(t,e,n):cu(t,e,n,o)}async function cu(t,e,n,o){let{delegate:r,lc:s}=e,i=`Recovering mutations from db ${t.name}.`;s.debug?.("Start:",i);try{let a=bo(t.replicacheFormatVersion),l=o||await H(n,c=>q(c)),u=new Set;for(;l;){let c;for(let[d,p]of l){if(r.closed){s.debug?.("Exiting early due to close:",i);return}if(!u.has(d)&&(u.add(d),c=await lu(p,d,n,t,e,a),c))break}l=c}}catch(a){_t(a,s,i,r)}s.debug?.("End:",i)}async function pu(t,e,n){let{delegate:o,lc:r}=e,s=`Recovering mutations from db ${t.name}.`;r.debug?.("Start:",s);try{let i=bo(t.replicacheFormatVersion),a=await H(n,u=>ee(u)),l=new Set;for(;a;){let u;for(let[c,d]of a){if(o.closed){r.debug?.("Exiting early due to close:",s);return}if(!l.has(c)&&(l.add(c),u=await du(d,c,n,t,e,i),u))break}a=u}}catch(i){_t(i,r,s,o)}r.debug?.("End:",s)}function si(t){return G(t)||K(t)}async function ii(t,e,n,o,r){G(o)?t.debug?.(`Client group ${e} cannot recover mutations for client group ${n}. The client group is unknown on the server. Marking it as disabled.`):K(o)&&t.debug?.(`Client group ${e} cannot recover mutations for client group ${n}. The client group's version is not supported on the server. versionType: ${o.versionType}. Marking it as disabled.`),await v(r,s=>dn(n,s))}async function du(t,e,n,o,r,s){y(o.replicacheFormatVersion>=5);let{delegate:i,lc:a,wrapInOnlineCheck:l,wrapInReauthRetries:u,isPushDisabled:c,isPullDisabled:d}=r,p=await r.clientGroupIDPromise;if(at(p),p===e)return;let f,g=!0;for(let[D,I]of Object.entries(t.mutationIDs))if(!t.lastServerAckdMutationIDs[D]||t.lastServerAckdMutationIDs[D]<I){f=D,g=!1;break}if(g)return;if(t.disabled){a.debug?.(`Not recovering mutations for client group ${e} because group is disabled.`);return}let m=`Recovering mutations for client group ${e}.`;a.debug?.("Start:",m);let S=new Oe(n,ai,Io,B);try{if(await v(S,R=>R.setHead(N,t.headHash)),c()){a.debug?.(`Cannot recover mutations for client group ${e} because push is disabled.`);return}let{pusher:D}=i,I="recoveringMutationsPush";if(!await l(async()=>{let{result:R}=await u(async(L,J)=>{y(f),y(S);let oe=await Ut(L,S,J,await i.profileID,e,f,D,o.schemaVersion,et);return{result:oe,httpRequestInfo:oe?.httpRequestInfo}},I,a);if(!R)return!1;let b=R.response;return si(b)?(await ii(a,p,e,b,n),!1):R.httpRequestInfo.httpStatusCode===200},I)){a.debug?.(`Failed to recover mutations for client ${e} due to a push error.`);return}if(d()){a.debug?.(`Cannot confirm mutations were recovered for client ${e} because pull is disabled.`);return}let{puller:w}=i,P="recoveringMutationsPull",x;if(!await l(async()=>{let{result:R}=await u(async(L,J)=>{y(f);let oe=await Kn(await i.profileID,f,e,o.schemaVersion,w,L,S,s,J,!1);return{result:oe,httpRequestInfo:oe.httpRequestInfo}},P,a),{pullResponse:b}=R;return si(b)?(await ii(a,p,e,b,n),!1):!b||R.httpRequestInfo.httpStatusCode!==200?!1:(x=b,!0)},P)){a.debug?.(`Failed to recover mutations for client ${e} due to a pull error.`);return}return y(x),a.debug?.(`Client group ${p} recovered mutations for client group ${e}.  Details`,{mutationIDs:t.mutationIDs,lastServerAckdMutationIDs:t.lastServerAckdMutationIDs,lastMutationIDChanges:x.lastMutationIDChanges}),await v(n,async R=>{let b=await ee(R),L=b.get(e);if(!L)return b;y(x);let J={},oe=!1;for(let[rt,De]of Object.entries(x.lastMutationIDChanges))(L.lastServerAckdMutationIDs[rt]??0)<De&&(J[rt]=De,oe=!0);if(!oe)return b;let ot=new Map(b).set(e,{...L,lastServerAckdMutationIDs:{...L.lastServerAckdMutationIDs,...J}});return await cn(ot,R),ot})}catch(D){_t(D,a,m,i)}finally{await S.close(),a.debug?.("End:",m)}}var pr=class{name;onmessage=null;onmessageerror=null;constructor(e){this.name=e}addEventListener(){}removeEventListener(){}dispatchEvent(){return!1}close(){}postMessage(){}},Kt=typeof BroadcastChannel>"u"?pr:BroadcastChannel;function hu(t){return`replicache-new-client-group:${t}`}function mu(t){return`replicache-new-client-group-v1:${t}`}function fu(t){return typeof t=="object"&&typeof t.clientGroupID=="string"&&typeof t.idbName=="string"}function ui(t,e,n,o,r,s,i){if(n.aborted)return;let a=new Kt(mu(t));if(r){a.postMessage({clientGroupID:o,idbName:e});let l=new Kt(hu(t));l.postMessage([o]),l.close()}a.onmessage=async l=>{let{data:u}=l;if(fu(u)){let{clientGroupID:c,idbName:d}=u;if(c!==o)if(d===e)await H(i,async f=>await Re(c,f)!==void 0)&&s();else{s();return}}},n.addEventListener("abort",()=>a.close())}function yu(t){return`replicache-on-persist:${t}`}function gu(t){M(t),k(t.clientGroupID),k(t.clientID)}function ci(t,e,n){if(e.aborted)return()=>{};let o=new Kt(yu(t));return o.onmessage=r=>{let{data:s}=r;gu(s),n({clientGroupID:s.clientGroupID,clientID:s.clientID})},e.addEventListener("abort",()=>{o.close()}),r=>{e.aborted||(o.postMessage(r),n(r))}}async function pi(t){let e=await ze(N,t);return(await Pn(e,t)).map(o=>({id:o.meta.mutationID,name:o.meta.mutatorName,args:o.meta.mutatorArgsJSON,clientID:o.meta.clientID})).reverse()}var Du=7*24*60*60*1e3,Su=5*60*1e3,di;function hi(t,e,n,o){ie("ClientGC",()=>(di=Iu(t,e),di),()=>Su,n,o)}function Iu(t,e){return v(e,async n=>{let o=Date.now(),r=await q(n),s=Array.from(r).filter(([a,l])=>a===t||o-l.heartbeatTimestampMs<=Du);if(s.length===r.size)return r;let i=new Map(s);return await ye(i,n),i})}var Cu=5*60*1e3,mi;function fi(t,e,n){ie("ClientGroupGC",()=>(mi=wu(t),mi),()=>Cu,e,n)}function wu(t){return v(t,async e=>{let n=await q(e),o=new Set;for(let s of n.values())$e(s),o.add(s.clientGroupID);let r=new Map;for(let[s,i]of await ee(e))(o.has(s)||pn(i))&&r.set(s,i);return await cn(r,e),r})}var xu=60*1e3,yi;function gi(t,e,n,o,r){ie("Heartbeat",async()=>{yi=bu(t,e);try{return await yi}catch(s){if(s instanceof Y){n();return}throw s}},()=>xu,o,r)}function bu(t,e){return v(e,async n=>{let o=await q(n),r=o.get(t);if(!r)throw new Y(t);let s={...r,heartbeatTimestampMs:Date.now()},i=new Map(o).set(t,s);return await ye(i,n),i})}var tt=class{#e=new Set;#t;constructor(e){this.#t=e}async visit(e){if(this.#e.has(e))return;this.#e.add(e);let n=await this.#t.mustGetChunk(e);await this.visitChunk(n)}async visitChunk(e){await Promise.all(e.meta.map(n=>this.visit(n)))}};var Wn=class extends tt{#e=new Map;#t;constructor(e){super(e),this.#t=e}get gatheredChunks(){return this.#e}visit(e){return this.#t.isMemOnlyChunkHash(e)?super.visit(e):U}visitChunk(e){return this.#e.set(e.hash,e),super.visitChunk(e)}};async function Ii(t,e,n,o,r,s,i,a=()=>Promise.resolve()){if(s())return;let[l,u,c]=await H(o,async m=>{await Ms(e,m);let S=await Xo(e,m);y(S,`No main client group for clientID: ${e}`);let[,D]=await Di(m,S),I=await D.getMutationID(e,m),C=await Ne(D,m);return le(C),[I,C,S]});if(s())return;let[d,p,f]=await H(n,async m=>{let S=await Ke(N,m),D=await Mn(S,{[e]:l||0},m),I=await Ne(S,m);le(I);let C;if(kt(I,u)>0){await a();let w=I.chunk.hash,P=new Wn(m);await P.visit(w),C=P.gatheredChunks}return[D,I,C]});if(s())return;let g=!1;await v(o,async m=>{let[S,D]=await Di(m,c),I=D.chunk.hash,C={...S.mutationIDs},{lastServerAckdMutationIDs:w}=S;if(f){let x=await Et(e,m);$e(x);let V=await Ne(D,m);if(le(V),kt(p,V)>0){g=!0,await Promise.all(Array.from(f.values(),b=>m.putChunk(b))),await Tt(e,{...x,persistHash:p.chunk.hash},m),I=p.chunk.hash;let R=await Pn(S.headHash,m);w=p.meta.lastMutationIDs,C={...w},I=await Si(R,I,m,r,C,t,i)}}I=await Si(d,I,m,r,C,t,i);let P={...S,headHash:I,mutationIDs:C,lastServerAckdMutationIDs:w};await Dt(c,P,m)}),f&&g&&await v(n,m=>m.chunksPersisted([...f.keys()]))}async function Di(t,e){let n=await Re(e,t);return y(n,`No client group for clientGroupID: ${e}`),[n,await O(n.headHash,t)]}async function Si(t,e,n,o,r,s,i){for(let a=t.length-1;a>=0;a--){let l=t[a],{meta:u}=l,c=await O(e,n);await l.getMutationID(u.clientID,n)>await c.getMutationID(u.clientID,n)&&(r[u.clientID]=u.mutationID,e=(await Un(l,n,e,o,s,u.clientID,i)).chunk.hash)}return e}var $n=class extends tt{#e=new Map;#t=0;#n;#o;#r;constructor(e,n,o,r=fe){super(e),this.#n=n,this.#o=o,this.#r=r}get gatheredChunks(){return this.#e}visit(e){return this.#t>=this.#o||this.#n.isCached(e)?U:super.visit(e)}visitChunk(e){if(this.#t<this.#o){let n=this.#r(e);this.#e.set(e.hash,{chunk:e,size:n}),this.#t+=n}return super.visitChunk(e)}};var Ru=5*2**20,ku=300;async function wi(t,e,n,o,r,s,i,a){if(i())return;let l=await H(e,d=>is(N,d));le(l);let u=await e.withSuspendedSourceCacheEvictsAndDeletes(async()=>{let d=await v(n,async D=>{let I=await Os(o,D);if(!I)throw new Y(o);let C=I.headHash,P=await(await O(C,D)).getMutationID(o,D),x=await Et(o,D);$e(x);let V=await $(C,D);if(le(V),Ci(l,V,C))return;let R=new $n(D,e,Ru);await R.visit(C);let{gatheredChunks:b}=R,L=new Set(x.refreshHashes);L.add(C);let J={...x,refreshHashes:[...L]};return await Tt(o,J,D),[C,V,P,b,x.refreshHashes]});if(i()||!d)return{type:"aborted"};if(await ce(ku),i())return{type:"aborted"};let[p,f,g,m,S]=d;return v(e,async D=>{let I=await Ke(N,D),C=await Ne(I,D);if(le(C),Ci(C,f,p))return{type:"aborted",refreshHashesForRevert:S};let w=await Mn(I,{[o]:g},D),P=[];for(let{chunk:b,size:L}of m.values())P.push(D.putChunk(b,L));await Promise.all(P);let x=p;for(let b=w.length-1;b>=0;b--)x=(await Un(w[b],D,x,r,t,w[b].meta.clientID,a)).chunk.hash;let V=await O(x,D),R=await Is(I,V,D,s,a);return await D.setHead(N,x),{type:"complete",newMemdagHeadHash:x,diffs:R,newPerdagClientHeadHash:p}})});if(i())return;let c=d=>v(n,async p=>{let g={...await Et(o,p),refreshHashes:d};await Tt(o,g,p)});if(u.type==="aborted"){u.refreshHashesForRevert&&await c(u.refreshHashesForRevert);return}return await c([u.newPerdagClientHeadHash]),[u.newMemdagHeadHash,u.diffs]}function Ci(t,e,n){let o=kt(t,e);return o>0||o===0&&n===e.chunk.hash}import{resolver as Pu}from"@rocicorp/resolver";function xi(t){return new Promise(e=>{typeof requestIdleCallback=="function"?requestIdleCallback(()=>e(),{timeout:t}):setTimeout(()=>e(),t)})}var Bt=class{#e;#t;#n;#o;#r;#s=void 0;#a=void 0;#l=Promise.resolve();#c=Promise.resolve();constructor(e,n,o,r,s=xi){this.#e=e,this.#t=n,this.#n=o,this.#o=r,this.#r=s,this.#o.addEventListener("abort",()=>{let i=new Z("Aborted");this.#a?.reject(i),this.#s?.reject(i),this.#a=void 0,this.#s=void 0})}schedule(){return this.#o.aborted?Promise.reject(new Z("Aborted")):this.#s?this.#s.promise:(this.#s=Pu(),this.#h(),this.#s.promise)}async#h(){try{await this.#l}catch{}if(await this.#c,!!this.#s&&(await this.#r(this.#t),!!this.#s)){this.#c=Mu(this.#n,this.#o),this.#a=this.#s,this.#s=void 0;try{this.#l=this.#e(),await this.#l,this.#a?.resolve()}catch(e){this.#a?.reject(e)}this.#a=void 0}}};async function Mu(t,e){try{await ce(t,e)}catch(n){y(n instanceof Z)}}function bi(t,e,n){let o=setInterval(t,e);n.addEventListener("abort",()=>{clearInterval(o)})}async function dr(t,e,n,o){let r=await fetch(e,{method:t,body:n,headers:o});if(!r.ok)throw new Error(`Got ${r.status} fetching ${e}: ${await r.text()}`);return r}var vu=new Set,Ri=Symbol(),hr=class{#e;#t;#n=Ri;#o=vu;#r=[];onError;onDone;#s;constructor(e,n,o,r,s=W){this.#e=e,this.#t=n,this.onError=o,this.onDone=r,this.#s=s}hasIndexSubscription(e){for(let n of this.#r)if(n.options.indexName===e)return!0;return!1}invoke(e,n,o){return this.#e(e)}matches(e){for(let[n,o]of e)if(Nu(this.#o,this.#r,n,o))return!0;return!1}updateDeps(e,n){this.#o=e,this.#r=n}onData(e){(this.#n===Ri||!this.#s(this.#n,e))&&(this.#n=e,this.#t(e))}};var mr=class{#e;#t;#n;#o;onError=void 0;onDone=void 0;constructor(e,n){this.#e=e,this.#t=n.prefix??"",this.#n=n.indexName,this.#o=n.initialValuesInFirstDiff??!1}hasIndexSubscription(e){return this.#n===e}onData(e){e!==void 0&&this.#e(e)}invoke(e,n,o){let r=async(s,i,a,l)=>{let u;if(n===0){if(!this.#o)return;y(o===void 0);let p=[];for await(let f of e.scan({prefix:i,indexName:s}).entries())p.push({op:"add",key:f[0],newValue:f[1]});u=p}else{y(o);let p=o.get(s??"")??[];u=l(p)}let c=[],{length:d}=u;for(let p=Mi(u,i,a);p<d&&a(u[p]).startsWith(i);p++)c.push(u[p]);return n===0||c.length>0?c:void 0};return this.#n?r(this.#n,this.#t,s=>s.key[0],s=>ki(s,qe)):r(void 0,this.#t,s=>s.key,s=>ki(s,i=>i))}matches(e){let n=e.get(this.#n??"");return n===void 0?!1:Eu(n,this.#t,this.#n)}updateDeps(e,n){}};function ki(t,e){return t.map(n=>{let o=e(n.key);switch(n.op){case"add":return{op:"add",key:o,newValue:n.newValue};case"change":return{op:"change",key:o,oldValue:n.oldValue,newValue:n.newValue};case"del":return{op:"del",key:o,oldValue:n.oldValue}}})}var Yn=class{#e=new Set;#t=new Set;#n;#o;hasPendingSubscriptionRuns=!1;constructor(e,n){this.#n=e,this.#o=n}#r(e){return this.#e.add(e),this.#a(e),()=>this.#e.delete(e)}addSubscription(e,{onData:n,onError:o,onDone:r,isEqual:s}){let i=new hr(e,n,o,r,s);return this.#r(i)}addWatch(e,n){let o=new mr(e,n??{});return this.#r(o)}clear(){for(let e of this.#e)e.onDone?.();this.#e.clear()}async fire(e){let n=Ou(this.#e,e);await this.#s(n,1,e)}async#s(e,n,o){let r=[...e];if(r.length===0)return;let s=await this.#n(i=>Promise.allSettled(r.map(async a=>{let l=new Gn(i);try{return await a.invoke(l,n,o)}finally{a.updateDeps(l.keys,l.scans)}})));for(let i=0;i<r.length;i++){let a=r[i],l=s[i];l.status==="fulfilled"?a.onData(l.value):a.onError?a.onError(l.reason):this.#o.error?.("Error in subscription body:",l.reason)}}async#a(e){if(this.#t.add(e),!this.hasPendingSubscriptionRuns){this.hasPendingSubscriptionRuns=!0,await Promise.resolve(),this.hasPendingSubscriptionRuns=!1;let n=[...this.#t];this.#t.clear(),await this.#s(n,0,void 0)}}shouldComputeDiffs(){return this.#e.size>0}shouldComputeDiffsForIndex(e){for(let n of this.#e)if(n.hasIndexSubscription(e))return!0;return!1}};function Nu(t,e,n,o){if(n===""){for(let r of o)if(t.has(r.key))return!0}for(let r of e)if(Hu(r,n,o))return!0;return!1}function Hu(t,e,n){for(let o of n)if(Vu(t,e,o.key))return!0;return!1}function Vu(t,e,n){let{indexName:o="",limit:r,prefix:s,startKey:i,startExclusive:a,startSecondaryKey:l}=t.options;if(e!==o)return!1;if(!o)return r!==void 0&&r<=0?!1:!s&&!i?!0:!(s&&(!n.startsWith(s)||Pi(t,n))||i&&(a&&mn(n,i)||hn(n,i)||Pi(t,n)));if(!s&&!i&&!l)return!0;let[u,c]=qe(n);return!(s&&!u.startsWith(s)||l&&(a&&mn(u,l)||hn(u,l))||i&&(a&&mn(c,i)||hn(c,i)))}function Pi(t,e){let{inclusiveLimitKey:n}=t;return t.options.limit!==void 0&&n!==void 0&&Pe(e,n)}function*Ou(t,e){for(let n of t)n.matches(e)&&(yield n)}function Eu(t,e,n){if(e==="")return!0;let o=n?s=>qe(s.key)[0]:s=>s.key,r=Mi(t,e,o);return r<t.length&&o(t[r]).startsWith(e)}function Mi(t,e,n){return fn(t.length,o=>ke(e,n(t[o])))}var fr="";function Tu(){if(fr===""){let t=new Uint8Array(4);crypto.getRandomValues(t),fr=Array.from(t,e=>e.toString(16)).join("")}return fr}var yr=new Map;function gr(t){let e=yr.get(t);return e?(e++,yr.set(t,e)):(yr.set(t,0),e=0),`${t}-${Tu()}-${e}`}var Dr="14.2.2";var ju=401,zu=100*2**20,Ju=5*60*1e3,Gu=24*60*60*1e3,Uu=5*60*1e3;function Hi(t,e){return _u(t,e,7)}function _u(t,e,n){let o=`rep:${t}:${n}`;return e?`${o}:${e}`:o}var Ku=8,Bu=1e3,qu=1e3,Wu=500,$u=500,Xn=()=>{},Yu={type:"NewClientGroup"},Ir=class{pullURL;pushURL;auth;name;#e;#t;#n=!1;#o;#r=0;get idbName(){return Hi(this.name,this.schemaVersion)}schemaVersion;get#s(){return{name:this.idbName,replicacheName:this.name,replicacheFormatVersion:7,schemaVersion:this.schemaVersion}}#a=!1;#l=!0;#c=pe();#h;#u;#S;#f;#ee;#te;#x=null;#I={};mutate;#T=0;#A=0;#b;#C;pullInterval;pushDelay;#F;puller;pusher;#m;#p;#d;#y;#i;#g=new AbortController;#R=!1;#L;#j;#z;#ne=new Bt(()=>this.#q(),Bu,Wu,this.#g.signal);#J;#oe=new Bt(()=>this.#W(),qu,$u,this.#g.signal);#k;get requestOptions(){return this.#F}onSync=null;onClientStateNotFound=vi;onUpdateNeeded=vi;getAuth=null;constructor(e){let{name:n,logLevel:o="info",logSinks:r=[Au],pullURL:s="",auth:i,pushDelay:a=10,pushURL:l="",schemaVersion:u="",pullInterval:c=6e4,mutators:d={},requestOptions:p={},puller:f,pusher:g,licenseKey:m,experimentalCreateKVStore:S,indexes:D={}}=e;if(this.auth=i??"",this.pullURL=s,this.pushURL=l,typeof n!="string"||!n)throw new TypeError("name is required and must be non-empty");this.name=n,this.schemaVersion=u,this.pullInterval=c,this.pushDelay=a,this.puller=f??lo(this),this.pusher=g??Xs(this);let I=e,C=I.enableMutationRecovery??!0;this.#k=I.enableLicensing??!0,this.#L=I.enableScheduledPersist??!0,this.#j=I.enableScheduledRefresh??!0,this.#z=I.enablePullAndPushInOpen??!0,I.exposeInternalAPI&&I.exposeInternalAPI({persist:()=>this.#q(),refresh:()=>this.#W(),lastMutationID:()=>this.#r});let w=r.length===1?r[0]:new Lu(r);this.#i=new Fu(o,{name:n},w),this.#i.debug?.("Constructing Replicache",{name:n,"replicache version":Dr}),this.#e=new Yn(this.#Q,this.#i);let P=De=>gt(this.#i,De),x;S?(P=S,x=P(this.idbName)):x=P(this.idbName),this.#o=P,this.#y=new Ve(P),this.#d=new Ie(x,z,B),this.#p=new Oe(this.#d,zu,z,B);let V=nt();this.#u=V.promise,this.#m=m;let R=nt();this.#ee=R.promise;let b=nt();this.#te=b.promise;let{minDelayMs:L=Us,maxDelayMs:J=_s}=p;this.#F={maxDelayMs:J,minDelayMs:L};let oe=Js(At(),0,this.#g.signal);this.#b=new Ft(this.#i.withContext("PULL"),new jn(this,()=>this.#ae()),oe),this.#C=new Ft(this.#i.withContext("PUSH"),new zn(this,()=>this.#le())),this.mutate=this.#he(d);let ot=nt();this.#S=ot.promise;let rt=nt();this.#f=rt.promise,this.#t=new qn({delegate:this,lc:this.#i,enableMutationRecovery:C,wrapInOnlineCheck:this.#v.bind(this),wrapInReauthRetries:this.#N.bind(this),isPullDisabled:this.#K.bind(this),isPushDisabled:this.#B.bind(this),clientGroupIDPromise:this.#f}),this.#J=ci(this.name,this.#g.signal,De=>{this.#ce(De)}),this.#re(D,ot.resolve,rt.resolve,V.resolve,R.resolve,b.resolve)}async#re(e,n,o,r,s,i){let{clientID:a}=this;await Sr.get(this.name),await this.#y.getProfileID().then(n),await this.#y.putDatabase(this.#s);let[l,u,c,d]=await vs(a,this.#i,this.#d,Object.keys(this.#I),e,7);o(l.clientGroupID),await v(this.#p,f=>f.setHead(N,u)),this.#h=await Ys(this.#p,N),r(),await this.#se(s),this.#z&&(this.pull().catch(Xn),this.push().catch(Xn));let{signal:p}=this.#g;gi(a,this.#d,()=>{this.#w(a)},this.#i,p),hi(a,this.#d,this.#i,p),Fs(this.#y,this.#i,p),fi(this.#d,this.#i,p),ui(this.name,this.idbName,p,l.clientGroupID,d,()=>{this.#Y(Yu)},this.#d),bi(()=>this.#E(),Ju,p),this.#E(c),At()?.addEventListener("visibilitychange",this.#G),await this.#ie(i,this.#i,p)}#G=async()=>{this.#a||At()?.visibilityState==="visible"&&await this.#U()};async#U(){let{clientID:e}=this,n=await H(this.#d,o=>$o(e,o));return n||this.#w(e),!n}async#se(e){if(!this.#k){e(!0);return}if(!this.#m){await this.#P(this.#i,"license key ReplicacheOptions.licenseKey is not set",!0,e);return}if(this.#i.debug?.(`Replicache license key: ${this.#m}`),this.#m===it){this.#i.info?.("Skipping license check for TEST_LICENSE_KEY. You may ONLY use this key for automated (e.g., unit/CI) testing. See https://replicache.dev for more information."),e(!0),this.#x=setTimeout(async()=>{await this.#P(this.#i,"Test key expired",!0,e)},Uu);return}try{let n=await ro(dr,Wt,this.#m,this.#i);if(n.pleaseUpdate&&this.#i.error?.("You are using an old version of Replicache that uses deprecated licensing features. Please update Replicache else it may stop working."),n.status==="VALID")this.#i.debug?.("License is valid.");else{await this.#P(this.#i,`status: ${n.status}`,n.disable,e);return}}catch(n){this.#i.error?.(`Error checking license: ${n}`)}e(!0)}async#P(e,n,o,r){e.error?.(`** REPLICACHE LICENSE NOT VALID ** Replicache license key '${this.#m}' is not valid (${n}). Please run 'npx replicache get-license' to get a license key or contact hello@replicache.dev for help.`),o&&(await this.close(),e.error?.("** REPLICACHE DISABLED **")),r(!1)}async#ie(e,n,o){if(!this.#k||!this.#m||this.#m===it){e(!1);return}let r=async()=>{try{await oo(dr,Wt,this.#m,await this.profileID,n)}catch(s){this.#i.info?.(`Error sending license active ping: ${s}`)}};await r(),e(!0),ie("LicenseActive",r,()=>Gu,n,o)}get profileID(){return this.#S}get clientID(){return this.#c}get clientGroupID(){return this.#f}onOnlineChange=null;get online(){return this.#l}get closed(){return this.#a}async close(){this.#a=!0;let{promise:e,resolve:n}=nt();Sr.set(this.name,e),this.#g.abort(),At()?.removeEventListener("visibilitychange",this.#G),await this.#u;let o=[this.#p.close(),this.#d.close(),this.#y.close()];this.#b.close(),this.#C.close(),this.#e.clear(),this.#x&&clearTimeout(this.#x),await Promise.all(o),Sr.delete(this.name),n()}async#M(e,n){let o=this.#h;at(o),e!==o&&(this.#h=e,await this.#e.fire(n))}async#_(e,n){for(;;){if(this.#a)return;await this.#u;let{clientID:o}=this,r=this.#i.withContext("maybeEndPull").withContext("requestID",n),{replayMutations:s,diffs:i}=await oi(this.#p,r,e,o,this.#e,7);if(!s||s.length===0){await this.#M(e,i),this.#X();return}for(let a of s){this.#e.hasPendingSubscriptionRuns&&await Promise.resolve();let{meta:l}=a;e=await he(this.#p,u=>$s(a,u,e,Te,this.#I,r,Be(l)?l.clientID:o,7))}}}#ae(){return this.#K()?Promise.resolve(!0):this.#v(async()=>{try{this.#D(0,1);let{syncHead:e,requestID:n,ok:o}=await this.#ue();if(!o)return!1;e!==T&&await this.#_(e,n)}catch(e){throw await this.#O(e)}finally{this.#D(0,-1)}return!0},"Pull")}#K(){return this.#n||this.pullURL===""&&Lr(this.puller)}async#v(e,n){let o=!0;try{return await e()}catch(r){return r instanceof Qe||r instanceof Ee?(o=!1,this.#i.debug?.(`${n} threw:
`,r,`
with cause:
`,r.causedBy)):r instanceof Cr?this.#i.error?.(r):this.#i.info?.(`${n} threw:
`,r),!1}finally{this.#l!==o&&(this.#l=o,this.onOnlineChange?.(o),o&&this.#E())}}async#N(e,n,o,r=Xn,s=Xn){let{clientID:i}=this,a=0,l;o=o.withContext(n);do{let u=gr(i),c=o.withContext("requestID",u),{httpRequestInfo:d,result:p}=await e(u,c);if(l=p,!d)return{result:p,authFailure:!1};let{errorMessage:f,httpStatusCode:g}=d;if((f||g>=400)&&c.error?.(`Got error response doing ${n}: ${g}`+(f?`: ${f}`:"")),g!==ju)return{result:p,authFailure:!1};if(!this.getAuth)return{result:p,authFailure:!0};let m;try{await r(),m=await this.getAuth()}finally{await s()}if(m==null)return{result:p,authFailure:!0};this.auth=m,a++}while(a<Ku);return o.info?.("Tried to reauthenticate too many times"),{result:l,authFailure:!0}}#B(){return this.#n||this.pushURL===""&&Qs(this.pusher)}async#le(){if(this.#B())return!0;await this.#u;let e=await this.#S,{clientID:n}=this,o=await this.#f;return this.#v(async()=>{let{result:r}=await this.#N(async(a,l)=>{try{this.#D(1,0);let u=await Ut(a,this.#p,l,e,o,n,this.pusher,this.schemaVersion,et);return{result:u,httpRequestInfo:u?.httpRequestInfo}}finally{this.#D(-1,0)}},"push",this.#i);if(r===void 0)return!0;let{response:s,httpRequestInfo:i}=r;return K(s)?this.#H(s):G(s)&&await this.#V(),i.httpStatusCode===200},"Push")}#H(e){let n={type:e.error};e.versionType&&(n.versionType=e.versionType),this.#Y(n)}push({now:e=!1}={}){return Ni(this.#C.send(e))}pull({now:e=!1}={}){return Ni(this.#b.send(e))}async poke(e){await this.#u;let{clientID:n}=this,o=gr(n),r=this.#i.withContext("handlePullResponse").withContext("requestID",o),{pullResponse:s}=e;if(K(s)){this.#H(s);return}if(G(s)){await this.#V();return}let i=await lr(r,this.#p,E(e.baseCookie),s,n,7);switch(i.type){case 0:await this.#_(i.syncHead,o);break;case 2:throw new Error("unexpected base cookie for poke: "+JSON.stringify(e));case 1:break}}async#ue(){await this.#u;let e=await this.profileID,{clientID:n}=this,o=await this.#f,{result:{beginPullResponse:r,requestID:s}}=await this.#N(async(u,c)=>{let d=await Kn(e,n,o,this.schemaVersion,this.puller,u,this.#p,7,c);return{result:{beginPullResponse:d,requestID:u},httpRequestInfo:d.httpRequestInfo}},"pull",this.#i,()=>this.#D(0,-1),()=>this.#D(0,1)),{pullResponse:i}=r;K(i)?this.#H(i):G(r.pullResponse)&&await this.#V();let{syncHead:a,httpRequestInfo:l}=r;return{requestID:s,syncHead:a,ok:l.httpStatusCode===200}}async#q(){y(!this.#R),this.#R=!0;try{let{clientID:o}=this;if(await this.#u,this.#a)return;try{await Ii(this.#i,o,this.#p,this.#d,this.#I,()=>this.closed,7)}catch(r){if(r instanceof Y)this.#w(o);else if(this.#a)this.#i.debug?.("Exception persisting during close",r);else throw r}}finally{this.#R=!1}let{clientID:e}=this,n=await this.#f;y(n),this.#J({clientID:e,clientGroupID:n})}async#W(){await this.#u;let{clientID:e}=this;if(this.#a)return;let n;try{n=await wi(this.#i,this.#p,this.#d,e,this.#I,this.#e,()=>this.closed,7)}catch(o){if(o instanceof Y)this.#w(e);else if(this.#a)this.#i.debug?.("Exception refreshing during close",o);else throw o}n!==void 0&&await this.#M(n[0],n[1])}#$(){this.onClientStateNotFound?.()}#w(e){this.#i.error?.(`Client state not found on client, clientID: ${e}`),this.#$()}async#V(){let e=await this.#f;y(e),this.#n=!0,await v(this.#d,n=>dn(e,n)),this.#i.error?.(`Client state not found on server, clientGroupID: ${e}`),this.#$()}#Y(e){this.#i.debug?.(`Update needed, reason: ${e}`),this.onUpdateNeeded?.(e)}async#X(){this.#L&&await this.#Z("persist",this.#ne)}async#ce(e){this.#i.debug?.("Handling persist",e);let n=await this.#f;e.clientGroupID===n&&this.#pe()}async#pe(){this.#j&&await this.#Z("refresh from storage",this.#oe)}async#Z(e,n){try{await n.schedule()}catch(o){o instanceof Z?this.#i.debug?.(`Scheduled ${e} did not complete before close.`):this.#i.error?.(`Error during ${e}`,o)}}#D(e,n){this.#T+=e,this.#A+=n;let o=e+n,r=this.#T+this.#A;if(o===1&&r===1||r===0){let s=r>0;Promise.resolve().then(()=>this.onSync?.(s))}}subscribe(e,n){return typeof n=="function"&&(n={onData:n}),this.#e.addSubscription(e,n)}experimentalWatch(e,n){return this.#e.addWatch(e,n)}query(e){return this.#Q(e)}#Q=async e=>{await this.#u;let{clientID:n}=this;return H(this.#p,async o=>{try{let r=await Ss(o,7),s=new Jt(n,r,this.#i);return await e(s)}catch(r){throw await this.#O(r)}})};#de(e,n){return this.#I[e]=n,async o=>(await this.#me(e,n,o,performance.now())).result}#he(e){let n=Object.create(null);for(let o in e)n[o]=this.#de(o,e[o]);return n}async#me(e,n,o,r){let s=E(o??null);this.#e.hasPendingSubscriptionRuns&&await Promise.resolve(),await this.#u;let{clientID:i}=this;return he(this.#p,async a=>{try{let l=await ze(N,a),c=await Tn(l,e,s,null,a,r,i,7),d=new Ze(i,await c.getMutationID(),"initial",c,this.#i),p=await n(d,o);xe(c);let f=await c.getMutationID(),[g,m]=await c.commitWithDiffs(N,this.#e);return this.#r=f,this.#C.send(!1),await this.#M(g,m),this.#X(),{result:p,ref:g}}catch(l){throw await this.#O(l)}})}async#O(e){return e instanceof yt&&await this.#U()?new Y(this.clientID):e}#E(e){return this.#t.recoverMutations(e,this.#u,this.#d,this.#s,this.#y,this.#o)}experimentalPendingMutations(){return H(this.#p,pi)}},Sr=new Map;function vi(){typeof location<"u"&&location.reload()}var Cr=class extends Error{};async function Ni(t){let e=await t;if(e)throw e.error}export{ct as ExperimentalMemKVStore,ut as IDBNotFoundError,Ee as PullError,Ir as Replicache,it as TEST_LICENSE_KEY,jt as TransactionClosedError,nC as consoleLogSink,Hl as deleteAllReplicacheData,zs as dropAllDatabases,Nl as dropDatabase,Ui as filterAsyncIterable,lo as getDefaultPuller,Ye as isScanIndexOptions,Hi as makeIDBName,_l as makeScanResult,aa as mergeAsyncIterables,Dr as version};
